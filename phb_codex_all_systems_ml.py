...
if os.path.exists("symbols.json"):
    with open("symbols.json") as f:
        SYMBOL_SUCCESS = json.load(f)
else:
    SYMBOL_SUCCESS = {s: 0.5 for s in SYMBOL_POOL}

def codex_attempt_adaptive():
    """Adaptive codex attempt biased toward DA_VINCI_CODEX symbols."""
    seq = []
    for i, target in enumerate(DA_VINCI_CODEX):
        bias = 0.7 if random.random() < 0.5 else 0.5
        if random.random() < bias:
            seq.append(target)
        else:
            seq.append(random.choice(SYMBOL_POOL))
    return seq

def update_symbol_success(seq, unlocked):
    for s in seq:
        if unlocked:
            SYMBOL_SUCCESS[s] = min(SYMBOL_SUCCESS.get(s,0.5) + 0.05, 1.0)
        else:
            SYMBOL_SUCCESS[s] = max(SYMBOL_SUCCESS.get(s,0.5) - 0.02, 0.01)
...
def simulate_system(system_name, steps=60, dt=1.0, attempts_per_step=200, seed=None):
    ...
    for step in range(steps):
        ...
        for _ in range(attempts_per_step):
            seq = codex_attempt_adaptive()
            if near_match(seq):
                seq = correct_toward_codex(seq)
            unlocked = codex_test(seq)
            update_symbol_success(seq, unlocked)
            if unlocked:
...
# -----------------------------
def main():
    print("\n=== PHB ALL SYSTEMS SIMULATION (ML-ADAPTIVE) ===")
    for system in PHB_SYSTEMS:
        ...
        print_base_codex()
    # save adaptive learning
    with open("symbols.json","w") as f:
        json.dump(SYMBOL_SUCCESS, f)
